...
You can create an alias for the server to reduce typing `username@server` every time
...
You can do this for multiple servers, so you `config` file might look like this.
...
To avoid re-entering your password every time when you log in to the same serve
