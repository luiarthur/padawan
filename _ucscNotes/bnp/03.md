...
$$
  \mathcal X = \\{ Y(\omega,t): \omega \in \Omega, t\in T \\}
$$
such that:
...
1.  For any $n$ and any set of index points $t\_i \in T, i = 1,...,n,
    ...
    $t\_i \in T$, $Y\_{t\_i}(\cdot) \equiv Y(\cdot, t\_i) : (\Omega, \mathcal F, P)
    \rightarrow S$ is a random function.)

2.  For any fixed $\omega \in \Omega$, $Y\_\omega(\cdot) \equiv Y(\omega, \cdot):
...
$\Omega$. The $\omega$ is the input to the function $Y$ while $t$ is the realization
index (or in a time series problem, time).

The Gaussian Process (GP) is therefore a SP.
