...
### Comparison between BNP \& Heirarchical Modeling
I was still having a hard time identifying the differences between a
heirarchical bayesian model and a BNP model. So, I decided to write out the
...
In the heirarchical model, once again, F($\theta$) could be N($\theta$,0.1),
...
Recall that according to Bayes' theorem, we can express a posterior distribution
as
$$ \begin{array}{rclcl}
      y &|& x,\theta &\sim& Normal(\theta,1)\\\\
      y &|& x,\theta &\sim& Normal(\theta,1)\\\\
\end{array} $$
